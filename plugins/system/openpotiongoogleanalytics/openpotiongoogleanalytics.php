			...
			if ($domain_model > 0) {
				switch ($domain_model) {
					case 1:
						...
						break;
					case 2:
						$ga_options = $ga_options . '_gaq.push([\'_setDomainName\', \'none\']);' . "\n";
...
';
		...
		switch ($insertion) {
			case 1: $pos = strrpos($buffer, "</body>"); break;
			case 2: $pos = strpos($buffer, ">", strpos($buffer, "<body")) + 1; break;
			default: $pos = strrpos($buffer, "</head>");
		}
		
		if ($pos > 0) {
			$buffer = substr($buffer, 0, $pos) ."\n". $google_analytics_javascript . substr($buffer, $pos);
